...
def make_averaged(original_function, times_called=1000):
    ...
    def func(*args):
        step, num = 1, 0
        while step <= times_called:
            num = num + original_function(*args)
            step = step + 1
        return num/times_called
    return func
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    high, idx, ans = 0, 1, 1
    while idx <= 10:
        cur = make_averaged(roll_dice, times_called)(idx, dice)
        if cur > high:
            high = cur
            ans = idx            
        idx = idx + 1
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    if sus_update(0, score, opponent_score) - score >= threshold:
        # print(sus_update(0, score, opponent_score))
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
...
def final_strategy(score, opponent_score, goal=100):
    """
    This strategy is based on comparing the results for different rolls, 
    to select the most reasonable rolls based on the comparison. 
    """
    boar_gain = max(1, boar_brawl(score, opponent_score))
    sus_gain  = sus_points(score + boar_gain) - score

    if score + boar_gain >= goal:
        return 0
    if score + 1 >= goal:
        return 1

    if boar_gain >= 8 or sus_gain >= 12:
        return 0
